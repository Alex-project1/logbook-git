...
export function AdditionalAlarmReasonsPage() {
    ...
    async function handleRestore(reason: AdditionalAlarmReason) {
        ...
        try {
            ...
            if (err.response?.status === 409) {
                setError("Нельзя восстановить: активная причина с таким названием уже существует");
            } else {
            ...
            <div className="content-grid">
                {!showArchive && (
                    <form className="panel-card" onSubmit={handleSubmit}>
                        <h2>
                            {editingReason ? "Редактировать причину" : "Добавить причину"}
                        </h2>

                        {editingReason?.isSystem && (
                            <div className="info-box">
                                Это системная причина. Можно изменить название, порядок и статус,
                                но нельзя удалить причину.
                            </div>
                        )}
                ...
                {showArchive && (
                    <div className="panel-card">
                        <h2>Архив причин</h2>
                        <div className="info-box">
                            Здесь отображаются удаленные обычные причины доп. сработок. Системные
                            причины в архив не попадают — их можно только включать или отключать.
                        </div>
  ...
  {showArchive ? (
    ...
    <RowActionMenu
      items={[
        ...
        {
          label: reason.isActive ? "Отключить" : "Включить",
